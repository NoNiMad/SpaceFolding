...
import fr.nonimad.microdimensions.network.PacketHandler;
...
public class AnchorBlock extends BlockContainer
{
	...
	public boolean onBlockActivated(World world, int x,	int y, int z, EntityPlayer player, int side, float dx, float dy, float dz) {
		...
		if(!DimensionManager.isDimensionRegistered(dimId)) {
			MicroDimensions.dimensions.add(dimId);
			DimensionManager.registerDimension(dimId, MicroDimensions.providerId);
			MinecraftServer.getServer().getConfigurationManager().sendPacketToAllPlayers(PacketHandler.getPacket(dimId));
		}
		
		if(DimensionManager.getWorld(dimId) == null)
			DimensionManager.initDimension(dimId);
		
		tile.setDimensionId(dimId);
